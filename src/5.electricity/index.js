...
function drawScene(gl, programInfos, buffers, textures, datas) {
  const { delta, mPos, framebufferTextures, electricitys, particles, size } = datas;
  {
    ...
    electricitys.forEach((electricity, index) => {
      VectorE.set(electricity.posEnd, ...mPos);
      const r = Vector.distance(electricity.posStart, electricity.posEnd);
      if (r < 400) {
        //for (let i = 0; i < 10; i++) {
        if (Math.random() > 0.7) {
      ...
      useTexture(gl, framebufferTextures.lineColor, false);
      shaderProgram.draw(gl.LINE_STRIP, bufferData.positionBufferData.data.length / 2);

      shaderProgram.attribSet({
        vertexColor: bufferData.lineWidthBufferData.buffer,
      });
      useTexture(gl, framebufferTextures.lineWidth, false);
      shaderProgram.draw(gl.LINE_STRIP, bufferData.positionBufferData.data.length / 2);
    });
  }
